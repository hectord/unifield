...
(function($){
	...
	var defaults = {
		...
		"icons" : {
            ...
            "collapse": "ui-icon-carat-1-n",
            "uncollapse": "ui-icon-carat-1-s",
		},
		"events" : {
			"load" : null,
			"beforeCollapse" : null,
	...
	var methods = {
		...
		"collapse" : function(){
			...
			$(self)
				...
				.dialog("widget")
                    ...
                    .find("#ui-dialog-collapse-button").addClass(settings.icons.uncollapse).removeClass(settings.icons.collapse).end()
		...
		"_initButtons" : function(){
			...
			$(titlebar)
				...
				.append('<a class="ui-dialog-titlebar-collapse ui-corner-all" href="#"><span id="ui-dialog-collapse-button" class="ui-icon '+settings.icons.collapse+'">collapse</span></a>')
				...
				.find(".ui-dialog-titlebar-maximize,.ui-dialog-titlebar-minimize,.ui-dialog-titlebar-collapse,.ui-dialog-titlebar-restore")
					.attr("role", "button")
				...
				.find(".ui-dialog-titlebar-minimize")
					...
					.css({ "right" : settings.maximize ? "4.6em" : settings.minimize ? "2.3em" : "-9999em" })
					.click(function(e){
						e.preventDefault();
						$(self).dialogExtend("minimize");
					})
				.end()
				.find(".ui-dialog-titlebar-collapse")
					.toggle(settings.collapse)
					.css({ "right" : settings.collapse ? (2.3*(1+ (settings.maximize?1:0) + (settings.minimize?1:0)))+"em" : "-9999em" })
					.click(function(e){
						e.preventDefault();
                        if ($(self).data("dialog-state") == "collapsed") {
                            $(self).dialogExtend("restore");
                        } else {
    						$(self).dialogExtend("collapse");
                        }
					})
				.end()
		...
		"_initStyles" : function(){
			...
			style += '.ui-dialog .ui-dialog-titlebar-collapse,';
		...
		"_toggleButtons" : function(){
			...
			$(self).dialog("widget")
                ...
                .find("#ui-dialog-collapse-button")
                    ...
                    .removeClass(settings.icons.uncollapse)
                .end();
			//maintain chainability
			return self;
		},
