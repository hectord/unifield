...
class threshold_value_rules_report(osv.osv):
    _name = 'threshold.value.rules.report'
    _rec_name = 'rule_id'
    _auto = False
    ...
    def _get_supplier_lt(self, cr, uid, ids, field_name, args, context=None):
        res = {}
        
        for line in self.browse(cr, uid, ids, context=context):
            res[line.id] = line.product_id.procure_delay/30.00
        
        return res
    
    _columns = {
        'rule_id': fields.many2one('threshold.value', string='Rules reference', readonly=True),
        ...
        'frequency_id': fields.float(digits=(16,2), string='Frequency', readonly=True),
        'threshold_value': fields.float(digits=(16,2), string='Threshold value', readonly=True),
        'manual_qty': fields.float(digits=(16,2), string='Manual Qty', readonly=True),
        'safety_stock': fields.float(digits=(16,2), string='Safety stock', readonly=True),
        'fixed_lt': fields.float(digits=(16,2), string='Fixed LT', readonly=True),
        'product_lt': fields.function(_get_supplier_lt, method=True, type='float', string='Product LT', readonly=True),
    ...
    def init(self, cr):
        ...
        '''
        tools.drop_view_if_exists(cr, 'threshold_value_rules_report')
        cr.execute("""
            CREATE OR REPLACE view threshold_value_rules_report AS (
                SELECT 
                    ...
                    min.frequency AS frequency_id,
                    min.safety_month AS safety_stock,
                    min.lead_time AS fixed_lt,
                    min.compute_method AS compute_method,
                    CASE WHEN min.compute_method = 'computed' THEN min.consumption_method ELSE '' END consumption_method,
                ...
                FROM
                    threshold_value_line line
                    LEFT JOIN
                        threshold_value min
                        ON
                        line.threshold_value_id = min.id
                    LEFT JOIN
