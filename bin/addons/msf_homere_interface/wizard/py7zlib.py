...
try:
    ...
    import logging
    logging.getLogger('homere').error('Unable to load pylzma library ! You will not be able to load a 7z Homere file.')
    ...
    def __init__(self, file, password=None):
        ...
        for idx in range(files.numfiles):
            ...
            if idx >= subinfo.numunpackstreams[fidx]+folder_start:
                ...
                # UFTP-333: line commented for UniField: the 7-zip files generated by Homere raise an error
                #src_pos = folder_pos
                folder_start = idx
                fidx += 1
