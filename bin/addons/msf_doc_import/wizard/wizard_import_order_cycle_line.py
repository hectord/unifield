...
class wizard_import_order_cycle_line(osv.osv_memory):
    _name = 'wizard.import.order.cycle.line'
    _description = 'Import Order cycle lines from Excel sheet'
    ...
    _columns = {
        ...
        'rule_id': fields.many2one('stock.warehouse.order.cycle', string='Rule', required=True),
    ...
    def _import(self, dbname, uid, ids, context=None):
        ...
        rule_obj = self.pool.get('stock.warehouse.order.cycle')
        rule_line_obj = self.pool.get('stock.warehouse.order.cycle.line')
        line_with_error = []
        vals = {'rule_line_ids': []}
        ...
        for wiz_browse in self.browse(cr, uid, ids, context):
            ...
            for row in rows:
                ...
                try:
                    ...
                    to_write.update({'safety_stock': qty_value['product_qty'], 'error_list': qty_value['error_list'], 'warning_list': qty_value['warning_list']})
                    ...
                    if not to_write.get('uom_id') or to_write.get('uom_id') == obj_data.get_object_reference(cr, uid, 'msf_doc_import', 'uom_tbd')[1]:
                        raise osv.except_osv(_('Error'), '\n'.join(x for x in p_value['error_list']))

                    # Check if the UoM is compatible with the product
                    if to_write.get('product_id') and to_write.get('uom_id'):
                        uom_categ = uom_obj.browse(cr, uid, to_write.get('uom_id'), context=context).category_id.id
                    ...
                    if qty_value['product_qty'] and uom_value['uom_id']:
                        round_qty = self.pool.get('product.uom')._change_round_up_qty(cr, uid, uom_value['uom_id'], qty_value['product_qty'], 'safety_stock')
                        if round_qty.get('warning', {}).get('message'):
                            to_write.update({'safety_stock': round_qty['value']['safety_stock']})
    ...
    def import_file(self, cr, uid, ids, context=None):
        ...
        for wiz_read in self.read(cr, uid, ids, ['rule_id', 'file']):
            ...
            try:
                ...
                res, res1 = wiz_common_import.check_header_values(cr, uid, ids, context, header_index, columns_for_order_cycle_line_import)
                if not res:
...
Otherwise, you can continue to use Unifield.""")
    ...
    def cancel(self, cr, uid, ids, context=None):
        ...
        return {'type': 'ir.actions.act_window',
                'res_model': 'stock.warehouse.order.cycle',
                'view_type': 'form',
                'view_mode': 'form, tree',
                'target': 'crush',
                'res_id': rule_id,
...
wizard_import_order_cycle_line()
