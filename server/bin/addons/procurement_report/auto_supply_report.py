...
class auto_supply_rules_report(osv.osv):
    ...
    def init(self, cr):
        ...
        '''
        tools.drop_view_if_exists(cr, 'auto_supply_rules_report')
        cr.execute("""
            CREATE OR REPLACE view auto_supply_rules_report AS (
                SELECT 
                ...
                FROM
                    stock_warehouse_automatic_supply min
                    LEFT JOIN
                        stock_warehouse_automatic_supply_line line
                        ON
                        line.supply_id = min.id
                    LEFT JOIN
