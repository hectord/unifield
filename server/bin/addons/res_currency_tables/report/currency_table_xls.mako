...
<%
    ...
    if data and data.get('form'):
        if data.get('form').get('header'):
            nb_of_columns = len(data.get('form').get('header'))
%>
<Table x:FullColumns="1" x:FullRows="1">
<Column ss:AutoFitWidth="1" ss:Width="240" />
% for x in range(1,nb_of_columns):
<Column ss:AutoFitWidth="1" ss:Width="120" />
% endfor
% if data.get('form').get('header'):
<Row>
% for value in data.get('form').get('header'):
...
<Cell ss:StyleID="ssBoldCenter">
   <Data ss:Type="String">${line[0]}</Data>
</Cell>
% for rate in line[1:]:
<Cell ss:StyleID="ssBorderCenter">
   <Data ss:Type="Number">${rate}</Data>
</Cell>
% endfor
</Row>
% endfor
