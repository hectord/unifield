...
class wizard_import_threshold_value_line(osv.osv_memory):
    ...
    _description = 'Import Threshold value lines from Excel sheet'
    ...
    _columns = {
        ...
        'rule_id': fields.many2one('threshold.value', string='Rule', required=True),
    ...
    def _import(self, dbname, uid, ids, context=None):
        ...
        rule_obj = self.pool.get('threshold.value')
        rule_line_obj = self.pool.get('threshold.value.line')
        line_with_error = []
        vals = {'rule_line_ids': []}
        ...
        for wiz_browse in self.browse(cr, uid, ids, context):
            ...
            for row in rows:
                ...
                to_write = {
                    ...
                    'default_code': False,
                    'product_qty': 0.00,
                    'fixed_product_qty': 0.00,
                    'fixed_threshold_value': 0.00,
                    'threshold_value_id': rule_id,
                    'threshold_value_id2': rule_id,
                }
                line_num += 1
                col_count = len(row)
                template_col_count = len(header_index.items())
                if col_count != template_col_count:
                    message += _("""Line %s in the Excel file: You should have exactly %s columns in this order: %s \n""") % (line_num, template_col_count,','.join([_(f) for f in columns_for_order_cycle_line_import]))
                ...
                try:
                    ...
                    to_write.update({'fixed_product_qty': qty_value['product_qty'], 'error_list': qty_value['error_list'], 'warning_list': qty_value['warning_list']})

                    # Cell 5: Fixed threshold value
                    thr_value = {}
                    thr_value = check_line.quantity_value(product_obj=product_obj, row=row, to_write=to_write, cell_nb=cell_index['threshold_value'], context=context)
                    to_write.update({'fixed_threshold_value': thr_value['product_qty'], 'error_list': qty_value['error_list'], 'warning_list': qty_value['warning_list']})



                    # Check rounding of qty according to UoM
                    if qty_value['product_qty'] and uom_value['uom_id']:
                        round_qty = self.pool.get('product.uom')._change_round_up_qty(cr, uid, uom_value['uom_id'], qty_value['product_qty'], 'fixed_product_qty')
                        if round_qty.get('warning', {}).get('message'):
                            to_write.update({'fixed_product_qty': round_qty['value']['fixed_product_qty']})
                            message += _("Line %s in the Excel file: %s\n") % (line_num, round_qty['warning']['message'])

                    # Check rounding of qty according to UoM
                    if thr_value['product_qty'] and uom_value['uom_id']:
                        round_qty = self.pool.get('product.uom')._change_round_up_qty(cr, uid, uom_value['uom_id'], thr_value['product_qty'], 'fixed_threshold_value')
                        if round_qty.get('warning', {}).get('message'):
    ...
    def import_file(self, cr, uid, ids, context=None):
        ...
        for wiz_read in self.read(cr, uid, ids, ['rule_id', 'file']):
            ...
            try:
                ...
                res, res1 = wiz_common_import.check_header_values(cr, uid, ids, context, header_index, columns_for_threshold_line_import)
                if not res:
...
wizard_import_threshold_value_line()
